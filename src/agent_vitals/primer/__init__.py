...
# Canonical mapping of host name -> MCP config path. Used by drift detection
# and `av install` to enumerate hosts without re-doing the scan each time.
HOST_PATHS: dict[str, Path] = {
    "pi":          Path.home() / ".pi" / "agent" / "mcp.json",
    "Claude Code": Path.home() / ".claude" / ".mcp.json",
    "Cursor":      Path.home() / ".cursor" / "mcp.json",
    "OpenCode":    Path.home() / ".config" / "opencode" / "mcp.json",
